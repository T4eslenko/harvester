        ...
        for dialog in dialogs:
            try:
                if isinstance(dialog.entity, User) and not dialog.entity.bot:
                    ...
                    user_dialogs.append(
                        f'👉{i}) <b>{first_name} {last_name} {username}</b> (id: {user.id}) [💬 <b>{count_messages}</b>]')
...
async def make_list_of_channels(delgroups, chat_message_counts, openchannels, closechannels, openchats, closechats, selection, client):
    ...
    for closechannel in closechannels:
        ...
        # Используем чистый текст без ANSI escape-кодов
        channels_list.append(f'👉{i}) <b>{closechannel.title}</b> [🧍 {closechannel.participants_count}, 💬 <b>{messages_count_for_harvester}</b>]')        
        closechannel_count += 1
    ...
    for closechat in closechats:
        ...
        channels_list.append(f'👉{i}) <b>{closechat.title}</b> [🧍 {closechat.participants_count}, 💬 <b>{messages_count_for_harvester}</b>]')     
        closegroup_count += 1
