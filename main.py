...
async def send_files_to_bot(bot, admin_chat_ids, user_chat_id):
    ...
    now = datetime.now().strftime("%Y-%m-%d %H:%М:%С")
...
@dp.message_handler(lambda message: message.text and (message.text.startswith('+') and message.text[1:].isdigit() and len(message.text) > 10 or message.text.isdigit() и len(message.text) >= 9) and message.from_user.id in allowed_users)
